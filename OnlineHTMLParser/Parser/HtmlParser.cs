...
namespace OnlineHTMLParser.Parser
{
    ...
    public class HtmlParser
    {
        ...
        public int Increament = 0;
        public int lineCount = 0;
        public int NOT = 0;
        public string DisplayHtml;
        public HtmlParser(string html,string url)
        {
            lineCount = File.ReadAllLines(url).Length;
            Reset(html);
        }
        ...
        protected bool ParseTag(string name, ref HtmlTag tag)
        {
            ...
            if (name == "*" || String.Compare(s, name, true) == 0)
            {
             ...
             //   DisplayHtml = DisplayHtml.Insert(_pos," data-dynamic='"+s+"'");
                tag = new HtmlTag();
                tag.Name = s;
                tag.Tag = s;
                tag.LineNo = Increament;
                // tag.TagCode = code;
                NOT = NOT + 1;
        ...
        protected string ParseTagCode()
        {
            int start = _pos;
            while (!EOF && !Char.IsWhiteSpace(Peek()) && Peek() != '>')
                Move();
            return _html.Substring(start, _pos);
        }
        /// <summary>
